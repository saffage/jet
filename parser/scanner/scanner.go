...
import (
	...
	"github.com/saffage/jet/parser/scanner/base"
...
func (s *Scanner) Next() token.Token {
	if !s.Match('\000') {
		...
		case s.Match('\'') && token.IsIdentStartChar(s.LookAhead(1)):
			s.Advance()
			ident := s.TakeWhile(token.IsIdentChar)
		...
		if !tok.Start.IsValid() {
			tok.Start = token.RangePos{
				Offset: startPos.Offset,
				Line:   startPos.Line,
				Char:   startPos.Char,
			}
			tok.FileID = startPos.FileID
		}

		if !tok.End.IsValid() {
			pos := s.PrevPos()
			tok.End = token.RangePos{
				Offset: pos.Offset,
				Line:   pos.Line,
				Char:   pos.Char,
			}
	...
	return token.Token{
		Kind:  token.EOF,
		Range: token.RangeFrom(s.Pos(), s.Pos()),
	}
}
...
func (s *Scanner) scanString() token.Token {
	...
	if !s.Consume(quote) {
		...
		return token.Token{
			Kind:  token.Illegal,
			Data:  data,
			Range: token.RangeFrom(quotePos, s.PrevPos()),
		}
	}
