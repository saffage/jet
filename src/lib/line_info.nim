...
func merge*(self, other: LineInfo): LineInfo
  {.raises: [ValueError].} =
  if self.line != other.line:
    raise newException(ValueError,
      &"operands must have the same line, got {self.line} and {other.line}")

  let (start, length) =
    if self.column < other.column:
      (self.column, (other.column + other.length) - self.column)
    else:
      (other.column, (self.column + self.length) - other.column)

  result = LineInfo(line: self.line, column: start, length: length)
