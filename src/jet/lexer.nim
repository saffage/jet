...
func parseUntil(s: openArray[char]; charSet: set[char]): string =
    result = ""
    discard s.parseUntil(result, charSet)

func parseUntil(s: openArray[char]; c: char): string =
    result = parseUntil(s, {c})
...
func lexComment(buffer: openArray[char]): LexResult =
    ...
    result.token.unsafeValue().data = buffer
        ...
        .parseUntil(Newlines)
    result.parsed = skipPrefix + result.token.unsafeValue().data.len().uint
